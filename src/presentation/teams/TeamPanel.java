...
public class TeamPanel extends MyPanel {
	...
	class RegularGamePanel extends MyPanel {
		...
		private String fieldList[] = { "numOfWin", "numOfLose", "avgAge",
				"point", "shot", "three", "two", "free", "totRebound",
				"assist", "steal", "block", "fault", "foul", "minute",
				"totalHit", "totalShot", "threeHit", "threeShot", "twoShot",
				"twoHit", "freeHit", "freeShot", "offRebound", "defRebound",
				"offEFF", "defEFF", "pace", "freeEFF", "threeEFF", "realShot",
				"shotEFF", "faultEFF", "offReboundEFF", "defReboundEFF" };// 数据展示
			...
			rangeAndNameTable.addMouseListener(new MouseListener() {
				...
				public void mouseClicked(MouseEvent arg0) {
					if (rangeAndNameTable.getSelectedRow() >= 0
							&& rangeAndNameTable.getSelectedRow() < rangeAndNameTable
									.getRowCount()) {
						int row = rangeAndNameTable.getSelectedRow();
						String teamName = (String) rangeAndNameTable
								.getValueAt(row, 2);
						String season = (String) rangeAndNameTable.getValueAt(
								row, 3);
						generalTeam generalTeam = teamInfoBl.getGeneralTeam(
								teamName, Season.getSeason(season));
		...
		private void initTable() {
			ArrayList<SeasonTeam> seasonTeamList = this.teamInfoBl
					.getSeasonTeam(Season.this_season, DataKind.average,
							League.all_league, Field.numOfWin);
		...
		private void fillTable(ArrayList<SeasonTeam> voList) {
			...
			if (voList != null) {
				for (int i = 0; i < voList.size(); i++) {
					Object performRow[] = voList.get(i)
							.getSpeContent(fieldList);
					...
					if (generalTeam != null) {
						logo = Method.changeSize(
								new ImageIcon(PathOfFile.TEAM_LOGO_IMAGE
										+ generalTeam.getImgName() + ".png"),
								60, 50);
					}
					Object infoRow[] = { String.valueOf(i + 1), logo,
							voList.get(i).getTeamName(),
							voList.get(i).getSeason() };
		...
		private void setTableStyle() {
			...
			teamShowTable.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {
						public void valueChanged(ListSelectionEvent arg0) {
							checkSelection(false);
						}
					});
			rangeAndNameTable.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {
						public void valueChanged(ListSelectionEvent arg0) {
							checkSelection(true);
						}
			...
			teamShowPane.setCorner(JScrollPane.UPPER_LEFT_CORNER,
					rangeAndNameTable.getTableHeader());
		...
		private void checkSelection(boolean isFixedTable) {
			...
			if (fixedSelectedIndex != selectedIndex) {
				if (isFixedTable) {
					teamShowTable.setRowSelectionInterval(fixedSelectedIndex,
							fixedSelectedIndex);
				} else {
					rangeAndNameTable.setRowSelectionInterval(selectedIndex,
							selectedIndex);
				}
			}
		}
		...
		class SelectionPanel extends MyPanel implements MouseListener {
			...
			private MyComboBox<Object> seasonChoose, dataKindChoose,
					leagueChoose, sortFieldChoose;
			...
			private void createObjects() {
				...
				findTeamButton = new JButton(new ImageIcon(
			...
			private void setComponentsLocation() {
				...
				dataKindChoose.setLocation(Jcombobox_x, Jcombobox_y
						+ Jcombobox_gap);
				leagueChoose.setLocation(Jcombobox_x, Jcombobox_y
						+ Jcombobox_gap * 2);
				sortFieldChoose.setLocation(Jcombobox_x, Jcombobox_y
						+ Jcombobox_gap * 3);
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getSource().equals(searchButton)) {
					...
					ArrayList<SeasonTeam> seasonTeamList = teamInfoBl
							.getSeasonTeam(Season.all_seasons[seasonInt],
									DataKind.dataKinds[dataKindInt],
									League.leagues[leagueInt],
